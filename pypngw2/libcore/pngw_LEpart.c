...
COMPLEX16 (*spart_LEFunc_fList[171])(BBHDynVariables*, BBHDynParams*)={
	NULL,
	NULL,
	NULL,
	spart_LEPartsl1_km3,
...
COMPLEX16 (*cpart_LEFunc_fList[171])(BBHDynVariables*, BBHDynParams*)={
	NULL,
	NULL,
	NULL,
	cpart_LEPartsl1_km3,
...
COMPLEX16 calculate_QLEPart_waveform(REAL8 lval, BBHDynVariables *var, BBHDynParams *pms)
{
    ...
    for (INT i=0; i<n_scpart_Func; i++) {
		...
		if (cpart_LEFunc_fList[i] != NULL)
			ret += cpart_LEFunc_fList[i](var, pms)*cos((nl + nk*k)*lval);
		if (spart_LEFunc_fList[i] != NULL)
...
COMPLEX16 calculate_QLEPart_waveform_emode(INT ilk, REAL8 lval, BBHDynVariables *var, BBHDynParams *pms)
{
	...
	if (cpart_LEFunc_fList[ilk] != NULL)
		ret += cpart_LEFunc_fList[ilk](var, pms)*cos((nl + nk*k)*lval);
	if (spart_LEFunc_fList[ilk] != NULL)
		ret += spart_LEFunc_fList[ilk](var, pms)*sin((nl + nk*k)*lval);
    return ret;
}
		...
		COMPLEX16 *ret_QPlus, COMPLEX16 *ret_QMinus)
{
	...
	if (cpart_LEFunc_fList[ilk] != NULL)
		Qc = cpart_LEFunc_fList[ilk](var, pms);
	if (spart_LEFunc_fList[ilk] != NULL)
		Qs = spart_LEFunc_fList[ilk](var, pms);
